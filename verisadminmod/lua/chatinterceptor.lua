    ...
    if vam.config.interceptProhibitedMessages == true then
        for key, useless in pairs(vam.data.prohibitedWordList) do
            if message:find(key) == true then
                Game.SendDirectChatMessage(vam.config.servermessageprefix, vam.config.messageInterceptionText, nil, 2, client)
                Game.Log(tostring(client) .. "'s message was intercepted by filter", 9)
                return true
            end
        end
    end
